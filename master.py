...
# -----------------------------------------------------------------------------
# Hilfsfunktionen (robustes Parsing)
# -----------------------------------------------------------------------------
...
async def save_df_text(df: pd.DataFrame, table: str):
    """
    Speichert DataFrame als TEXT-Spalten.
    Optimiert: executemany() statt execute() je Zeile, saubere Platzhalter.
    """
    conn = await get_conn()
    try:
...
async def neukontakte_preview(
    ...
    try:
        ...
        for p in sel:
            ...
            # NEU: robustes Email-Parsing (kein "or []" mehr)
            emails_raw = p.get("email")
            email_list = _as_list_email(emails_raw)
