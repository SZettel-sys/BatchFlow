# master_merged_full.py
# Vollständige integrierte Version: master_20251030 (Neukontakte) + Nachfass-Logik aus master_20251103
# Beide Modi: /neukontakte?mode=new|nachfass|refresh
# Abgleich: Orga-Match ≥95 % via RapidFuzz (gegen Filter 1245/851/1521) + Person-ID in 1216/1708
# Tabellen: nk_master_final/ready/delete_log und nf_master_final/ready/delete_log
# JS/f-string-Klammern korrigiert (keine Platzhalter).
...
def is_forbidden_activity_date(val: Optional[str]) -> bool:
    # keine zukünftigen Daten, keine in den letzten 3 Monaten
...
# Aktivitäts-Feldkeys (best effort)
...
async def stream_counts_with_org_cap(filter_id: int, fachbereich_key: str, per_org_limit: int, page_limit: int = PAGE_LIMIT):
    ...
    while True:
        ...
        if not chunk:
            break
        for p in chunk:
            # Aktivitätslogik für Nachfass/Neukontakte
...
# =============================================================================
# UI – Gemeinsame Seite /neukontakte
# =============================================================================
...
async def neukontakte(request: Request, mode: str = Query("new")):
    ...
    default_per_org_js = PER_ORG_DEFAULT_LIMIT  # numerisch injizieren
  ...
  <section class="card">
    <div style="display:grid;grid-template-columns:repeat(12,1fr);gap:16px">
      ...
      <div style="grid-column:span 5">
        ...
        <div id="fbinfo" style="color:#64748b;font-size:13px">Die Zahl berücksichtigt bereits das Orga-Limit.</div>
      </div>
...
async function loadOptions() {{
  try {{
    ...
    el('fbinfo').textContent = "Gesamt: " + data.total + " · Fachbereiche: " + data.options.length;
    el('total-count').textContent = String(data.total);
    toggleCTAs();
  }} catch(e) {{
    alert('Fehler beim Laden der Optionen: ' + e);
  }}
}}
loadOptions();
...
async def neukontakte_options(per_org_limit: int = Query(PER_ORG_DEFAULT_LIMIT, ge=1, le=3), mode: str = Query("new")):
    ...
    _OPTIONS_CACHE[key] = {"ts": now, "total": total, "options": options}
    return JSONResponse({"total": total, "options": options})

# =============================================================================
# Datenaufbau & Export-Hilfen
# =============================================================================
...
async def _build_master_final_from_pd(
    ...
    await save_df_text(df, f"{table_prefix}_master_final")
    return df

# =============================================================================
# Abgleich – Orga≥95 % & Person-ID
# =============================================================================
...
async def _reconcile_impl(table_prefix: str) -> HTMLResponse:
    ...
    if master.empty:
        return HTMLResponse(f"<div style='padding:24px'>Keine Daten in {table_prefix}_master_final.</div>")
    ...
    # Orga-Dubletten (Filter 1245/851/1521) mit Kappung
...
def _df_to_excel_bytes(df: pd.DataFrame) -> bytes:
    ...
    from openpyxl.utils import get_column_letter  # noqa: F401 (nur für Leserlichkeit)
    buf = io.BytesIO()
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("master_merged_full:app", host="0.0.0.0", port=int(os.getenv("PORT", 8000)), reload=False)
