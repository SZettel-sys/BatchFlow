# master.py — BatchFlow (FastAPI + Pipedrive + Neon) — mem-safe full build
...
# OOM-Schutz (für Orga-Namen-Sammlung)
MAX_ORG_NAMES = int(os.getenv("MAX_ORG_NAMES", "120000"))
MAX_ORG_BUCKET = int(os.getenv("MAX_ORG_BUCKET", "15000"))

# Nachfass – Memory/Performance Limits (safe defaults)
NF_MAX_ROWS    = int(os.getenv("NF_MAX_ROWS", "20000"))  # harte Kappung
NF_PAGE_LIMIT  = int(os.getenv("NF_PAGE_LIMIT", "200"))  # kleinere Pages
...
<main>
  <div class="grid">
    ...
    <div class="card"><div class="title">Nachfass</div><div class="desc">Folgekampagne für bereits kontaktierte Leads.</div><a class="btn" href="/nachfass">Starten</a></div>
...
# =============================================================================
# Nachfass – FELDER & HILFSFUNKTIONEN
# =============================================================================
...
async def _build_nf_master_final(nf_batch_ids: List[str], batch_id: str, campaign: str) -> pd.DataFrame:
    ...
    selected: List[dict] = []
    total_sel = 0
    async for chunk in stream_persons_by_filter(FILTER_NACHFASS, page_limit=NF_PAGE_LIMIT):
        for p in chunk:
            ...
            selected.append(p)
            total_sel += 1
            if NF_MAX_ROWS and total_sel >= NF_MAX_ROWS:
                break
        if NF_MAX_ROWS and total_sel >= NF_MAX_ROWS:
            break
...
async def _reconcile_nf() -> None:
    ...
    # Orga-Dubletten via ≥95 % (Filter 1245/851/1521) – mit Kappung
    filter_ids_org = [1245, 851, 1521]
    buckets_all: Dict[str, List[str]] = {}
    collected_total = 0
    for fid in filter_ids_org:
    ...
    suspect_ids = set()
    async for page in stream_person_ids_by_filter(1216, PAGE_LIMIT):
        suspect_ids.update(page)
    async for page in stream_person_ids_by_filter(1708, PAGE_LIMIT):
        suspect_ids.update(page)
    ...
    log_df = pd.DataFrame(delete_rows, columns=["reason", "id", "name", "org_id", "org_name", "extra"])
    await save_df_text(log_df, "nf_delete_log")

# =============================================================================
# Nachfass – Export-Jobs & Summary
# =============================================================================
class NFJob(Job):
    pass

@app.post("/nachfass/export_start")
async def nf_export_start(
  ...
  <section class="card">
    <div class="grid">
      <div class="col-6">
        ...
        <div class="hint">Komma- oder zeilengetrennt. Es wird auf Teilstring im Feld „Batch ID“ gematcht.</div>
      </div>
      <div class="col-3">
        <label>Batch ID (Exportspalte)</label>
        <input id="batch_id" placeholder="Bxxx"/>
        <div class="hint">z. B. B222</div>
      </div>
      <div class="col-3">
        <label>Kampagnenname</label>
...
async function startExportNf() {
  const raw = el('nf_batch_ids').value || '';
  const ids = _parseIDs(raw);
  const bid = el('batch_id').value || '';
  const camp= el('campaign').value || '';
  if (ids.length === 0) { alert('Bitte mindestens eine Batch ID angeben.'); return; }
  ...
  if (!r.ok) { const msg = await r.text().catch(()=> ''); hideOverlay(); alert('Start fehlgeschlagen: ' + (msg || r.status)); return; }
  const { job_id } = await r.json();
  await pollNf(job_id);
}

async function pollNf(job_id) {
  let done = false, tries = 0;
  while (!done && tries < 3600) {
    await new Promise(res => setTimeout(res, 300));
  ...
  if (done) {
    ...
    setTimeout(() => {
      window.location.href = '/nachfass/summary?job_id=' + encodeURIComponent(job_id);
    }, 800);
  } else {
    el('phase').textContent = 'Zeitüberschreitung beim Export';
  }
